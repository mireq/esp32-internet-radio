			  ...
			  struct sideinfo *si, unsigned int nch)
{
  ...
  for (gr = 0; gr < ngr; ++gr) {
    ...
    enum mad_error error;
		/*
	for (size_t g = 0; g < 5576; ++g) {
		mad_fixed_t value = abs(xr[0][g] / (MAD_F_ONE / 256));
		mad_fixed_t value = xr[0][g];
		printf("%lu %d\r", g, 0);
		for (size_t i = 0; i < 256; ++i) {
			if (i >= value) {
				if (g == 575) {
					(*framebuffer)[255-i][g] = 0xff444444;
				}
				else {
					(*framebuffer)[255-i][g] = 0xff000000;
				}
			}
			else {
				(*framebuffer)[255-i][g] = 0xffffffff;
			}
		}
	}
		*/
